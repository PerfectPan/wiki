...
5. 不再向 `log.md` 追加新记录；有意义的知识变更应在 PR body 中说明日期、受影响页面和来源指针。
...
1. 在执行任意命令前，先阅读 `AGENTS.md`、`SCHEMA.md`，必要时补读 `index.md`。`log.md` 只作为历史材料按需查阅，不作为新增记录目标。
