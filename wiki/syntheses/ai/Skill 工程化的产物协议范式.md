...
- **垫图是可判定的**：`model_capabilities.json` 的默认值是「能力未知 + 必须垫图」，只有被标定过的模型才允许仅靠名字或特征激活；未知模型不许猜，也不许编造特征；
- **参考图隔离写成提示词的一部分**：只抽线条、笔触、媒介、材质、色彩倾向，不得沿用主体、构图、文字、故事；并且图文模式下必须把路径、上传说明和隔离块**移出**可复制提示词；
...
- **产物协议硬且分层**：MP4 + SRT + 原稿 + 独立配音 + `timeline.json` + 可编辑工程 ZIP + 验证报告；项目文件布局、timeline schema 和 finalize manifest schema 都写死；
- **反冒充是一等约束**：两条路线不可混称，SVG 动效不得称为逐笔绘制，缺片的样片不得称为完整片，不假装完成无法验证的成片；
- **参数按职责分离**：`hand-follow` 只平滑手部显示位置（不改变笔迹速度），`hand-height` 管遮挡，笔迹速度由音频窗口与源图复杂度决定——文档明确禁止靠「调小 hand-follow」或降帧去解决速度问题；
- **时间轴不变量**：区域被完全扣空时要占满原时间窗，不放静止假手、不让后续区域提前；
- **失败恢复按类型分流**：预检失败只阻止依赖该能力的阶段，已通过的音频、插画、字幕不重做；上游快照固定 commit 且故意不含上游 SKILL.md，避免包内出现第二个可触发 Skill；
- **验收分层且诚实**：13 条含反例的 evals、四层验收、逐笔 10 条检查与「必须看边界帧」的规定；媒体检查工具同时声明自己不做语音识别、不判语义同步、不评美感，退出码 0 只代表本次自动检查无错误。
