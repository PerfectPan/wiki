...
- **annotation（标注）/ 区域 / 义岛**：人在源图上框出的一块块待绘制区域；「义岛」是左右分栏讲解时画面一侧独立成块的图文区。`protectedRegions` 是受保护、不被后画内容覆盖的区域。
- **扣空**：一块区域被后画上来的区域完全盖住，在它自己的时间段里实际看不见。
- **静止假手**：没有笔画动作时，一只画出来的手停在画面中央不动。
- **后续区域提前**：下一块区域没轮到自己的时间就先冒出来。
- **`skeleton` / `contour-wipe`**：渲染风格，skeleton 是只勾线稿的路径；contour-wipe 是让轮廓线按笔画顺序逐段显现。
