...
| `freeze.js`（889 B） | **MAIN** | **`document_start`** | 劫持时间原语，见 [[wiki/topics/frontend/页面时间冻结\|页面时间冻结]] |
...
**一个容易误判的点**：`content.js` 里有 20 多处 `__reactFiber$` / `__REACT_DEVTOOLS_GLOBAL_HOOK__` 命中，但它们全部出自**打包进来的 react-dom 自身源码**。反向探测「读取宿主页面框架实例」的典型写法（`Object.keys(el).find(k => k.startsWith('__react'))` 等）命中数为 **0**。结论：Voila **不读宿主页面的框架内部**，框架无关成立；它进入 MAIN world 仅用于时间控制。
...
表面上它有不少「糙」的地方，但每一处都能对上一个约束：

| 看起来糙 | 约束 |
| --- | --- |
| `!important` 在 content.js 里出现 118 次 | 改不了别人网站的原有 CSS 规则，只能靠优先级压过去 |
| 每次改动全量重写整段 `<style>` | 比增量 `insertRule` 更不易错，且撤销退化成「清空 textContent」，零簿记 |
| selector 允许不唯一、最多 5 层 | 运行时靠元素对象缓存干活，selector 只是交接物，短且可读比绝对唯一更重要 |
...
**值得借：[[wiki/topics/frontend/页面时间冻结|页面时间冻结]]。** 对拥有预览容器的产品，注入同款成本很低，能解掉 hover 态、下拉菜单、动画中途改不了的问题，并顺带解锁逐帧动画编辑。
