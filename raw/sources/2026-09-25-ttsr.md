...
1. A project native rule wins over a user native rule with the same filename-derived name; more generally, the first rule discovered for a name wins.
1. A valid rule with condition or astCondition is a TTSR rule even if it also says alwaysApply: true; it is not also injected as always-on context.
1. A rule's interruptMode overrides the global ttsr.interruptMode.
1. If several eligible rules match the same output, omp can inject all distinct matches. If any matching rule requires interruption, the response is interrupted.
``
...
## Related
- Context files — guidance that belongs in every prompt.
- Hooks — deterministic interception around tool execution.
- Skills — on-demand playbooks selected for a task.

---

## 附注：omp 项目背景与周边信号（2026-09-25 抓取，非官方文档原文）

### 项目基本信息（来源：GitHub API + omp.sh 落地页）

- 仓库 `can1357/oh-my-pi`，描述「⌥ Coding agent with the IDE wired in. Built by Stencil Labs.」，33,218 star（2026-09-25），主语言 TypeScript，创建于 2025-12-31，持续活跃（当日仍有提交）。
- 官网 omp.sh。原生 Rust 核心的 agentic harness，工具「against real sessions 调优」；Windows 原生（免 WSL），支持 macOS / Linux。
- 卖点「IDE wired in」：LSP（语言服务器）、调试器、浏览器、GitHub、持久记忆直接接入终端 agent。
- 社区资料称其为 Pi（mariozechner 所作）的分叉延伸；注意 GitHub 仓库本身并非 fork 关系。

### 周边信号（来源：网络搜索，2026-09-25）

- TTSR 概念为 omp 自有，无独立第三方讨论；相邻领域是 agent guardrails（框架层 hooks、运行时控制、pre/post-LLM 护栏分类等）。
- HN（2026-08-17，AGENTS.md 支持讨论帖）有用户评价：某主流 agent「模型没问题，但 harness 近半年可用性已明显落后于我在用的另一个（OMP）」——OMP 已被当作 harness 可用性对比标杆。
- 第三方报道：Medium《OMP: The Open-Source Coding Agent That Makes AI IDEs Look Outdated》；composio.dev《Pi vs OMP》。
- 官方文档 Related 区显示 omp 同时有 Hooks（工具执行周界的确定性拦截）与 TTSR（生成流中途拦截）两层机制，二者互补。
