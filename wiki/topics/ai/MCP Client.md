...
- 2025-03-29 的 journal 记录了一个很直接的实现心智模型：MCP Client 可以被看成“包一层 LLM function call”的执行器。
- 如果底层模型本身支持 function calling，client 可以直接把工具声明和调用结果接到模型 API。
- 如果模型本身不支持 function calling，也可以让模型产出约定格式的结构化文本，再由 client 先行 parse 后执行。journal 里举的是 XML 风格的协议，思路和很多代码生成工具用的自定义标记语言类似。
