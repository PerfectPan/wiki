...
`ai-cli` 源码里有几处比功能本身更值得借：

- 参数在 CLI 边界收窄成可靠类型，业务层不处理半脏数据。
- stdin 有首包超时探测，避免 agent 环境里误等 stdin 卡死。
- 并发生成使用固定 worker 数，并保持结果顺序稳定。
- 人类进度输出走 stderr，机器结果走 stdout，降低 agent 解析成本。
- 图片/视频预览是 best-effort，不阻塞文件产物生成。
- 动态模型目录只缓存成功结果，失败不永久缓存，允许下一次重试。
- 模型能力和调用路径分离，能表达 language-image 这类跨能力模型。
- 测试押在发布包入口、参数解析、模型目录、二进制输入输出、并发和预览底层这些容易坏的边界模块。

这些点比具体接哪家模型更有迁移价值。OpenClaw 如果要做类似能力，应当学习它的“命令协议”和“产物协议”，而不是直接继承它的 provider 绑定。
