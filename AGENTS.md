...
- `tooling`：广义工具类主题，包括开发工具、工程链路、编辑器、CLI、调试器、构建工具、文档格式、常用实用工具
...
7. 标题写概念名词，不写动作或状语。标题要能直接当 `[[双链]]` 的锚点和检索词：写「时间轴不变量」「参数的职责分离」，不写「时间轴上要写不变量」「参数分离比调参更重要」这类祈使句。正文用「X 是 Y」「X 的位置在 Y」式的判断句，少用「要把……」「必须先……」这种教程口吻的连续动词和状语。这是知识库写法，不是操作教程。
